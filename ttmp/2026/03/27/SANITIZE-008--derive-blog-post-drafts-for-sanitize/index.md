---
Title: Derive blog post drafts for sanitize
Ticket: SANITIZE-008
Status: active
Topics:
    - writing
    - documentation
DocType: index
Intent: long-term
Owners: []
RelatedFiles: []
ExternalSources: []
Summary: Ticket workspace for deriving blog-post candidates and structured article drafts for the sanitize project, with one draft file per article idea and reMarkable delivery.
LastUpdated: 2026-03-27T09:34:25.283085583-04:00
WhatFor: Organize, draft, validate, and deliver article-structure documents for writing about sanitize in Manuel's own voice later.
WhenToUse: Use when turning sanitize project research and code history into article-ready outlines, section plans, examples, and reference packs.
---

# Derive blog post drafts for sanitize

## Overview

This ticket packages a writing-oriented follow-up to the `sanitize` project work. The goal is not to write finished blog posts in assistant prose, but to leave behind article-ready draft files that capture section intent, section content, original repo resources, and promising examples or pseudocode for later human writing.

## Key Links

- Design doc: `design-doc/01-blog-post-outlines-and-article-drafting-plan-for-sanitize.md`
- Diary: `reference/01-investigation-diary.md`
- Article drafts:
  - `reference/02-from-regexes-to-a-real-sanitizer.md`
  - `reference/03-why-yaml-sanitizing-worked-better-than-json-repair.md`
  - `reference/04-building-a-conservative-repair-engine.md`
  - `reference/05-tree-sitter-as-infrastructure-not-a-magic-wand.md`
  - `reference/06-anatomy-of-the-yaml-pipeline.md`
  - `reference/07-the-json-recovery-story-useful-but-narrow.md`
  - `reference/08-designing-a-corpus-for-malformed-structured-text.md`
  - `reference/09-how-the-browser-playground-changed-the-project.md`
  - `reference/10-what-i-learned-trying-to-repair-llm-json.md`
  - `reference/11-a-weekend-project-that-turned-into-a-small-research-lab.md`
  - `reference/12-what-not-to-auto-fix.md`
  - `reference/13-how-to-write-span-aware-diagnostics-for-text-repair-tools.md`

## Status

Current status: **active**

## Topics

- writing
- documentation

## Tasks

See [tasks.md](./tasks.md) for the current task list.

## Changelog

See [changelog.md](./changelog.md) for recent changes and decisions.

## Structure

- `design-doc/` contains the article-set framing doc.
- `reference/` contains the diary plus one structured draft per article idea.
- `scripts/` is currently unused for this ticket because the work is documentation-heavy rather than experimental code.
- `various/` and `archive/` remain available if future writing notes or rejected article directions need to be stored.
